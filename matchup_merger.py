...
def gamelog_experience_cluster_merge(gamelog_df, experience_df, cluster_df, final_data=False):
    '''
    ...
    if final_data:
        filepath = '3_final_model_data/gamelog_exp_clust.pkl'
    else:
        filepath = '2_model_data/gamelog_{}_exp_clust.pkl'.format(str(window))

    print(filepath)
    df.to_pickle(filepath)

def final_stats_experience_cluster_merge(gamelog_df, experience_df, cluster_df, season=2018):
    '''
    INPUT: Gamelog DataFrame and experience DataFrame
    OUTPUT: DataFrame with matching IDs merged to same row (1 game per row!)
    '''
    ...
    filepath = '3_final_model_data/season{}_final_stats.pkl'.format(season)
    print(filepath)
    df.to_pickle(filepath)
...
if __name__ == '__main__':
    ...
    final_stats_2017_season = pd.read_pickle('0_scraped_data/season2017_final_stats_5_game_rolling.pkl')
    final_stats_2018_season = pd.read_pickle('0_scraped_data/season2018_final_stats_5_game_rolling.pkl')
    gamelog_roll5_df = pd.read_pickle('0_scraped_data/gamelog_data_5_game_rolling.pkl')
    team_experience_df = pd.read_pickle('1_player_data/team_experience.pkl')
    team_clusters_df = pd.read_pickle('1_player_data/team_clusters.pkl')
    odds_df = pd.read_pickle('other/odds/odds_data/odds_data.pkl')
    ...
    # gamelog_experience_cluster_merge(gamelog_roll2_df, team_experience_df, team_clusters_df)
    # gamelog_experience_cluster_merge(gamelog_roll3_df, team_experience_df, team_clusters_df)
    # gamelog_experience_cluster_merge(gamelog_roll4_df, team_experience_df, team_clusters_df)
    # gamelog_experience_cluster_merge(gamelog_roll5_df, team_experience_df, team_clusters_df)
    # gamelog_experience_cluster_merge(gamelog_roll6_df, team_experience_df, team_clusters_df)
    # gamelog_experience_cluster_merge(gamelog_roll7_df, team_experience_df, team_clusters_df)

    '''5 game rolling ave gave best stats'''
    # gamelog_experience_cluster_merge(gamelog_roll5_df, team_experience_df, team_clusters_df, final_data=True)
    final_stats_experience_cluster_merge(final_stats_2017_season, team_experience_df, team_clusters_df, season=2017)
    final_stats_experience_cluster_merge(final_stats_2018_season, team_experience_df, team_clusters_df, season=2018)
